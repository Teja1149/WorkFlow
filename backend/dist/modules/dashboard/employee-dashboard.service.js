...
export async function getEmployeeDashboard(organizationId, employeeId) {
    ...
    const dueSoon = items.filter((item) => item.deadline && item.deadline >= today && item.deadline <= inThreeDays && item.status !== 'DONE');
    const overdue = items.filter((item) => item.deadline && item.deadline < today && item.status !== 'DONE');
