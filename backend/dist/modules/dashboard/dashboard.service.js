...
export async function getManagerDashboard(organizationId, managerId) {
    ...
    const overdue = (workItems || []).filter((item) => item.deadline && item.deadline < today && item.status !== 'DONE').length;
    return {
        projects: projects || [],
        workItems: workItems || [],
        updates: updates || [],
