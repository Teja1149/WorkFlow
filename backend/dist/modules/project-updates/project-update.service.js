...
export async function createProjectUpdateTemplate(projectId, input) {
    if (!input?.createdBy) {
...
export async function addFieldsToTemplate(templateId, fields) {
...
export async function getProjectUpdateTemplate(projectId) {
...
export async function submitProjectDailyUpdate(projectId, employeeId, input) {
...
export async function getProjectDailyUpdates(projectId, filters = {}, viewer) {
    const normalizedRole = String(viewer.role || '')
        .toUpperCase();
    const canViewAll = normalizedRole === 'MANAGER' ||
...
export async function submitProjectTeamUpdate(projectId, organizationId, submittedBy, input) {
    const updateDate = input.updateDate ||
...
export async function getProjectTeamUpdates(projectId, filters) {
    let query = supabaseAdmin
        .from('project_team_updates')
        .select(`
      *,
